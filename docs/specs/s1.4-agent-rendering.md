---
title: "[P1] Agent rendering — pathfinding, speech bubbles, status indicators, click-to-select"
status: in_progress
issue: 17
priority: P1
tags: [stream-1, frontend, pixi, agents, pathfinding]
---

# S1.4 — Agent Rendering & Interaction

Enhanced agent sprite system with pathfinding, speech bubbles, status indicators, and click-to-select.

## Completed (PR #33)

- Agent sprite system with pixel art animations (idle, walking, talking)
- Random walk movement between walkable tiles
- Name labels above agent sprites
- Selection ring visual indicator
- Demo mode silly animations (dance, pee, poop, vomit, stab, sleep)

## Remaining Work

### Pathfinding
<!-- canon:system:1 status:done -->
- New file: `frontend/src/components/world/pixi/pathfinding.ts`
- Frontend pathfinding on tile grid, walkability check (grass/path = walkable; building/fence/water = blocked)
- Integrate into `AgentSprite.ts`, replacing random adjacent tile selection
- Path smoothing for natural movement
- Backend shortest-path movement helpers now exist in `backend/app/world/service.py` and support movement caps/proximity rules, but frontend route planning/rendering remains outstanding

**Acceptance Criteria:**
- [x] Frontend pathfinding finds shortest walkable path between any two tiles
<!-- canon:realized-in:PR#116 file:frontend/src/components/world/pixi/pathfinding.ts -->
- [x] Agents navigate around buildings, fences, and water
<!-- specwright:realized-in:PR#53 file:backend/app/world/service.py -->
<!-- specwright:realized-in:PR#53 file:backend/app/engine/service.py -->
<!-- canon:realized-in:PR#116 file:frontend/src/composables/usePathfinding.ts -->
- [ ] Movement looks natural with interpolated sprite position
- [x] Handles unreachable destinations gracefully
- [x] Unit tests cover edge cases
<!-- specwright:realized-in:PR#53 file:backend/tests/test_simulation_engine.py -->
<!-- canon:realized-in:PR#116 file:frontend/tests/unit/pathfinding.spec.ts -->
<!-- canon:realized-in:PR#116 file:frontend/tests/unit/usePathfinding.spec.ts -->

### Speech Bubbles
<!-- canon:system:2 status:in_progress -->
- New file: `frontend/src/components/world/pixi/SpeechBubble.ts`
- PixiJS Graphics + Text above agent sprite
- Auto-sizing bubble, fade in/out animation
- Styles: speech (white), thought (cloud), shout (jagged)
- Triggered by WebSocket `agent_action` events

**Acceptance Criteria:**
- [ ] Bubbles appear above agents with readable text
- [ ] Auto-size to fit content
- [ ] Fade in/out animation
- [ ] Different styles for speech/thought/shout
- [ ] Dismiss after configurable timeout (4s default)

### Status Indicators
<!-- canon:system:3 status:in_progress -->
- Extend `AgentSprite.ts` with overlay system
- Thinking spinner (rotating dots during LLM processing)
- Goal icon (cooperate/investigate/sabotage)
- Suspicion glow ring (green > yellow > orange > red, 0-100)

**Acceptance Criteria:**
- [ ] Thinking spinner during decision processing
- [ ] Goal icon displays objective type
- [ ] Suspicion glow changes color by level
- [ ] Updates reactively from store state

### Click-to-Select
<!-- canon:system:4 status:done -->
- PixiJS `pointerdown` events on `AgentSprite.ts`
- Emit to `usePixiWorld.ts` > updates `uiStore.selectedAgentId` + `showDossier`
- Highlight selected agent, click empty to deselect

**Acceptance Criteria:**
- [x] Click agent opens dossier
<!-- canon:realized-in:audit file:frontend/src/components/world/RelationshipWeb.vue:50 -->
- [x] Selected agent highlighted
<!-- canon:realized-in:audit file:frontend/src/components/world/pixi/AgentSprite.ts -->
- [x] Click empty deselects
- [x] Selection persists across pan/zoom

## HD Sprite System

> See `hd-sprite-system.md` — the agent sprite rendering is being upgraded from 14×18px to
> 32×48px composable characters. Impacts: sprite scale change affects selection ring/name label
> sizing, 26+ poses available, props render as part of sprite, `playAnimation()` API unchanged
> but more animations available.

## Key Files

`pixi/AgentSprite.ts`, `pixi/pathfinding.ts`, `pixi/SpeechBubble.ts`, `usePixiWorld.ts`
