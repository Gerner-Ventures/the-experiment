...
// Scale factor for pixel offsets — derived from tile size, future-proofs for other dimensions
const S = TILE_W / 64
const BASE_HEIGHT = 40 * S
...
export class BuildingRenderer {
  ...
  private getHeightForLocation(loc: LocationDef, theme: MapTheme): number {
    if (theme.buildingStyle === 'brutalist') {
      if (loc.type === 'meeting_hall' || loc.id === 'town_hall') return 80 * S
      if (loc.type === 'store' || loc.type === 'bar') return 65 * S
      return 55 * S + Math.min(loc.width, loc.height) * 8 * S
    }
    if (theme.buildingStyle === 'roman') {
      if (loc.id === 'town_hall') return 70 * S
      return BASE_HEIGHT + 5 * S
    }
    return BASE_HEIGHT
  }
  ...
  private drawHuts(c: Container, w: number, h: number, hw: number, hh: number, fill: string, stroke: string, highlight: string, loc: LocationDef) {
    ...
    for (let i = 0; i < Math.min(w, 3); i++) {
      ...
      g.moveTo(sx, 0)
      g.lineTo(sx, -height + 5 * S)
      g.stroke({ color: stiltColor, width: 2 * S, alpha: 0.8 })
    }
    ...
    g.fill(fill)
    g.stroke({ color: stroke, width: S })

    // Horizontal bamboo lines
    for (let i = 1; i < 4; i++) {
      const yLine = -5 * S - (height - 5 * S) * (i / 4)
      const yLineR = -bh / 2 - 5 * S - (height - bh / 2 - 5 * S + 5 * S) * (i / 4)
      g.moveTo(0, yLine)
      g.lineTo(bw / 2, yLineR)
      g.stroke({ color: stroke, width: 0.5 * S, alpha: 0.4 })
    }
    ...
    const overhang = 6 * S
    g.poly([
      -bw / 2 - overhang, -bh / 2 - height + 3 * S,
      0, -roofPeak,
      bw / 2 + overhang, -bh / 2 - height + 3 * S,
      0, -height + 3 * S,
    ])
    g.fill('#7a8a3a')
    g.stroke({ color: '#5a6a2a', width: S })

    // Leaf texture lines on roof
    g.moveTo(-bw / 4, -bh / 4 - height + 3 * S)
    g.lineTo(0, -roofPeak + 3 * S)
    g.stroke({ color: '#6a7a2a', width: S, alpha: 0.5 })
    g.moveTo(bw / 4, -bh / 4 - height + 3 * S)
    g.lineTo(0, -roofPeak + 3 * S)
    g.stroke({ color: '#6a7a2a', width: S, alpha: 0.5 })
  ...
  private drawWireframe(c: Container, w: number, h: number, hw: number, hh: number, loc: LocationDef) {
    ...
    const height = loc.id === 'town_hall' ? 60 * S : BASE_HEIGHT
    ...
    g.stroke({ color: dimColor, width: S, alpha: 0.4 })
    ...
    for (let i = 1; i < gridLines; i++) {
      ...
      g.stroke({ color: dimColor, width: 0.5 * S, alpha: 0.3 })
    }
    ...
    for (const [cx, cy] of corners) {
      ...
      g.stroke({ color, width: 1.5 * S, alpha: 0.8 })

      // Node dot at top
      g.circle(cx, cy - height, 2 * S)
    ...
    g.stroke({ color, width: S, alpha: 0.6 })
    ...
    for (let i = 0; i < streams; i++) {
      ...
      g.stroke({ color, width: 0.5 * S, alpha: 0.2 + Math.random() * 0.3 })
    }

    // Pulsing connection lines between top corners
    g.moveTo(0, -height)
    g.lineTo(bw / 2, -bh / 2 - height)
    g.stroke({ color, width: 0.5 * S, alpha: 0.3 })
    g.moveTo(0, -height)
    g.lineTo(-bw / 2, -bh / 2 - height)
    g.stroke({ color, width: 0.5 * S, alpha: 0.3 })
  ...
  private drawRoman(c: Container, w: number, h: number, hw: number, hh: number, fill: string, stroke: string, highlight: string, loc: LocationDef) {
    ...
    const height = isTownHall ? 70 * S : BASE_HEIGHT + 5 * S
    ...
    if (isTownHall) {
      ...
      for (let row = 0; row < archRows; row++) {
        ...
        for (let i = 0; i < archCount; i++) {
          ...
          g.stroke({ color: '#8a7050', width: S, alpha: 0.7 })
      ...
      g.fill('#e8d8b0')
      g.stroke({ color: stroke, width: 2 * S })
      ...
      for (let i = 0; i < crenels; i++) {
        ...
        g.rect(cx - 2 * S, cy - 4 * S, 4 * S, 4 * S)
        g.fill('#d4c4a0')
        g.stroke({ color: stroke, width: 0.5 * S })
      }
      ...
      for (let i = 0; i < cols; i++) {
        ...
        // Column shaft
        g.moveTo(cx, cy)
        g.lineTo(cx, cy - height + 4 * S)
        g.stroke({ color: '#c4b490', width: 3 * S, alpha: 0.8 })

        // Capital (top decoration)
        g.rect(cx - 3 * S, cy - height + 2 * S, 6 * S, 4 * S)
        g.fill('#e0d0b0')

        // Base
        g.rect(cx - 2 * S, cy - 2 * S, 4 * S, 3 * S)
        g.fill('#c4b490')
      }
      ...
      if (w >= 2) {
        ...
        g.moveTo(ax - 8 * S, ay + 10 * S)
        g.arcTo(ax, ay, ax + 8 * S, ay + 10 * S, 10 * S)
        g.lineTo(ax + 8 * S, ay + 10 * S)
        g.stroke({ color: '#aa9060', width: 1.5 * S, alpha: 0.6 })
      }
    }
  ...
  private drawBrutalist(c: Container, w: number, h: number, hw: number, hh: number, fill: string, stroke: string, highlight: string, loc: LocationDef) {
    ...
    const height = isTownHall ? 80 * S : isStore ? 65 * S : 55 * S + Math.min(w, h) * 8 * S
    ...
    const windowRows = Math.max(3, Math.floor(height / (18 * S)))
    ...
    for (let row = 0; row < windowRows; row++) {
      for (let col = 0; col < windowCols; col++) {
        ...
        g.rect(xBase - 2 * S, yBase + yOff - 2 * S, 4 * S, 5 * S)
    ...
    if (isTownHall || loc.id === 'workshop') {
      ...
      g.rect(stackX - 3 * S, stackY - height - stackH, 6 * S, stackH)
      g.fill('#2a2a2a')
      g.stroke({ color: '#1a1a1a', width: S })

      // Smoke puffs
      g.circle(stackX, stackY - height - stackH - 5 * S, 4 * S)
      g.fill({ color: '#555555', alpha: 0.3 })
      g.circle(stackX - 3 * S, stackY - height - stackH - 12 * S, 5 * S)
      g.fill({ color: '#555555', alpha: 0.2 })
      g.circle(stackX + 2 * S, stackY - height - stackH - 18 * S, 3 * S)
    ...
    if (w >= 2) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        g.stroke({ color: pipeColor, width: 2 * S, alpha: 0.6 })

        // Pipe joint
        g.circle(px, py - height * 0.35, 2 * S)
        g.fill(pipeColor)
      }
    }
    ...
    if (height >= 65 * S) {
      ...
      g.lineTo(0, -height - bh / 2 - 15 * S)
      g.stroke({ color: '#5a5a5a', width: S })
      g.circle(0, -height - bh / 2 - 15 * S, 1.5 * S)
